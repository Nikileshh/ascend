...
// Access model: a new account gets a 7-day free trial with FULL access; after
// that, app features are locked (402 via requireActiveAccess) until they go
// premium. Admins and premium users always have full access.
...
app.post(
  ...
  (req: AuthedRequest, res) => {
    ...
    if (!name || name.length > 40)
      return res
        .status(400)
        .json({ error: "Habit name (1-40 chars) required" });
    if (!["daily", "weekdays", "weekly"].includes(frequency))
      return res
        .status(400)
        .json({ error: "frequency must be daily, weekdays or weekly" });
    if (
      user.plan.habits.some((h) => h.name.toLowerCase() === name.toLowerCase())
    )
...
app.post(
  ...
  async (req: AuthedRequest, res) => {
    ...
    if (!win || !distraction || !lesson)
      return res
        .status(400)
        .json({ error: "win, distraction, lesson required" });
    try {
